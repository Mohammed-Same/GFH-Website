...
export default function LocationsPageClient() {
  ...
  const [visibleCount, setVisibleCount] = useState(8); // For pagination
  ...
  useEffect(() => {
    // Reset visibleCount when state filter changes
    setVisibleCount(8);
  }, [selectedState]);
  ...
  // For 'All States', show only visibleCount stores, else show all filtered
  ...
  const canShowMore =
    selectedState === "" && visibleCount < filteredStores.length;
  ...
  return (
    <section className="py-16 px-4 min-h-screen bg-white">
      <div className="max-w-6xl mx-auto">
        <h2
          className="text-5xl font-extrabold text-center mb-2 text-gray-900"
          style={{ fontFamily: "Inter, Arial, Helvetica, sans-serif" }}
        >
          Our Locations
        </h2>
        <p className="text-center text-lg text-gray-500 mb-10">
          Visit any of our convenient store locations for expert assistance with
          your wireless needs
        </p>
        ...
        <div className="flex flex-wrap justify-center gap-3 mt-8">
          <button
            ...
            className={`whitespace-nowrap px-6 py-2 rounded-full border-2 border-[#ff8200] text-base font-semibold transition-all duration-150 ${
              selectedState === ""
                ? "bg-[#ff8200] text-white"
                : "bg-white text-[#ff8200] hover:bg-orange-100"
            }`}
            style={{
              ...
              boxShadow:
                selectedState === ""
                  ? "0 4px 16px 0 rgba(255,130,0,0.10)"
                  : undefined,
            }}
            type="button"
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-12">
          {loading ? (
            ...
            paginatedStores.map((store, idx) => (
              <div
                key={idx}
                className="bg-white rounded-2xl shadow border border-gray-200 p-8 flex flex-col gap-2"
              >
        ...
        {canShowMore && (
          <div className="flex justify-center mb-12">
            <button
              ...
              className="px-8 py-3 rounded-full bg-[#ff8200] text-white font-semibold text-lg shadow hover:bg-orange-600 transition-all duration-150"
              style={{ fontFamily: "Inter, Arial, Helvetica, sans-serif" }}
              type="button"
            >
              See More
